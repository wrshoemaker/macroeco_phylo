...
for environment_idx, environment in enumerate(environments_to_keep):
    ...
    for rank_idx, rank in enumerate(diversity_utils.taxa_ranks):
        ...
        ratio_ = var_genera_all/var_asv_all

        x, y, z = plot_utils.get_scatter_density_arrays_for_loglog(var_asv_all, ratio_)
        ax.scatter(x, y, c=numpy.sqrt(z), cmap='Blues', s=70, alpha=0.9, edgecolors='none', zorder=1)

        
        min_ = min(var_asv_all)
        max_ = max(var_asv_all)
        #ax.plot([min_,max_],[min(ratio_),max(ratio_)], lw=3,ls=':',c='k',zorder=1)
        max_y = max([min(ratio_), max(ratio_)]) * 4 
...
fig_name = "%sfigure-2-figure-supplement-12-fine_vs_coarse_variance_ratio_taxon%s.png" % (config.analysis_directory, diversity_utils.get_rarefied_label(rarefied))
