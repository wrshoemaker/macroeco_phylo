...
dbd_dict = dbd_utils.load_svd_and_make_diversity_slm_dbd_simulation_phylo_dict()
...
for environment_idx, environment in enumerate(environments_to_keep):
    ...
    for coarse_rank_idx, coarse_rank in enumerate(phylo_distances_to_keep):
        ...
        predicted = dbd_dict[environment]['phylo'][coarse_rank]['slope_slm_all']
        ...
        ax.scatter(observed, predicted, color=color_phylo, s=40, linewidth=0.8, alpha=0.8, edgecolors='k', zorder=2)

        merged_ = numpy.concatenate((observed, predicted),axis=0)

        min_ = min(merged_)
        max_ = max(merged_)*1.2
...
fig.text(0.02, 0.5, "Predicted diversity slope, simulation", va='center', rotation='vertical', fontsize=28)



fig.subplots_adjust(hspace=0.3,wspace=0.35)
fig_name = "%sfigure-6-figure-supplement-2-dbd_slope_slm_simulation_diversity_phylo_all.png" % (config.analysis_directory)
