...
ax.plot([taxon_min,taxon_max], [taxon_min,taxon_max], lw=2,ls='--',c='k',zorder=1, label='1:1')
ax.set_xlim([taxon_min,taxon_max])
ax.set_ylim([taxon_min,taxon_max])
ax.set_xlabel("Observed mean diversity slope", fontsize = 12)
ax.set_ylabel("Predicted mean diversity slope, simulation", fontsize = 11)
ax.legend(loc="upper left", fontsize=7)
ax.set_title("Taxonomic coarse-graining", fontsize=12, fontweight='bold')
ax.text(0.2, 0.85, r'$\rho^{2} =$' + str(round(taxon_rho, 3)), fontsize=10, ha='center', va='center', transform=ax.transAxes)
...
fig_name = "%sfigure-6-figure-supplement-3-dbd_diversity_slm_simulation_mean_taxon.png" % (config.analysis_directory)
