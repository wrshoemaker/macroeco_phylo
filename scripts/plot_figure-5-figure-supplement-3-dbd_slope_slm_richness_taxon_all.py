...
dbd_dict = dbd_utils.load_richness_slm_dbd_dict()
...
for environment_idx, environment in enumerate(environments_to_keep):
    ...
    for coarse_rank_idx, coarse_rank in enumerate(diversity_utils.taxa_ranks[1:]):
        ...
        if coarse_rank in taxa_ranks:
            ...
            ax.scatter(observed, predicted, color=color_taxon, s=40, linewidth=0.8, edgecolors='k', zorder=2)
...
fig.text(0.02, 0.5, "Predicted richness slope", va='center', rotation='vertical', fontsize=28)



fig.subplots_adjust(hspace=0.3,wspace=0.35)
fig_name = "%sfigure-5-figure-supplement-3-dbd_slope_slm_richness_taxon_all.png" % (config.analysis_directory)
