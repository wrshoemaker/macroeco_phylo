...
do
    ...
    bash_out="/N/u/wrshoema/Carbonate/macroeco_phylo/scripts/bash/qsub_make_richness_diversity_prediction_phylo_dict_carbonate_${counter}.sh"
    if [ -f $bash_out ]; then
        rm $bash_out
    fi
    ...
    echo "python /N/u/wrshoema/Carbonate/macroeco_phylo/scripts/make_richness_diversity_prediction_phylo_dict_carbonate.py ${environment}" >> $bash_out
