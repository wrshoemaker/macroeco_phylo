...
do
    ...
    bash_out="/N/u/wrshoema/Carbonate/macroeco_phylo/scripts/bash/qsub_make_diversity_slm_phlyo_integral_dict_carbonate_${counter}.sh"
    if [ -f $bash_out ]; then
        rm $bash_out
    fi
    ...
    echo '#SBATCH --mem=30G' >> $bash_out

    echo 'module unload python' >> $bash_out
    echo 'module load anaconda' >> $bash_out
    echo 'conda activate macroeco_phylo' >> $bash_out

    echo "python /N/u/wrshoema/Carbonate/macroeco_phylo/scripts/run_make_diversity_slm_phlyo_integral_dict.py ${environment}" >> $bash_out
