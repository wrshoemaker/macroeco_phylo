...
declare -a measures=("richness" "diversity")
...
do
  ...
  do
    ...
    bash_out="/N/u/wrshoema/Carbonate/macroeco_phylo/scripts/bash/qsub_make_diversity_slope_taxon_dict_carbonate_${counter}.sh"
    if [ -f $bash_out ]; then
        rm $bash_out
    fi
    ...
    echo "python /N/u/wrshoema/Carbonate/macroeco_phylo/scripts/make_diversity_slope_taxon_carbonate.py ${distance} ${measure}" >> $bash_out
