...
do
  ...
  bash_out="/N/u/wrshoema/Carbonate/macroeco_phylo/scripts/bash/qsub_diversity_vs_diversity_phylo_carbonate_${counter}.sh"
  if [ -f $bash_out ]; then
      rm $bash_out
  fi
  ...
  echo "#SBATCH --job-name=env-${distance}" >> $bash_out
  ...
  echo "python /N/u/wrshoema/Carbonate/macroeco_phylo/scripts/make_diversity_vs_diversity_phylo_carbonate.py ${distance}" >> $bash_out
