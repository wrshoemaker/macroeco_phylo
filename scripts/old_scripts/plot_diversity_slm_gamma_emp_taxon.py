...
#predict_diversity_slm_emp.predict_diversity_gamma_taxon(diversity_utils.environments_to_keep[0], rarefied=False)
...
for environment_chunk_idx, environment_chunk in enumerate(environment_chunk_all):
    ...
    for environment_idx, environment in enumerate(environment_chunk):
        ...
        diversity_dict = predict_diversity_slm_emp.load_predict_diversity_gamma_taxon(environment)
...
fig.suptitle('Gamma AFD of each ASV simulated using the mean and variance', fontsize=14, y=0.95)


fig.subplots_adjust(hspace=0.3,wspace=0.35)
fig_name = "%sdiversity_slm_gamma_emp_taxon.png" % config.analysis_directory
