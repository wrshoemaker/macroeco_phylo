...
diversity_vs_diversity_dict = dbd_utils.load_diversity_vs_diversity_phylo_dict(rarefied=False)
...
for environment_chunk_idx, environment_chunk in enumerate(environment_chunk_all):
    ...
    for environment_idx, environment in enumerate(environment_chunk):
        ...
        distances = list(diversity_vs_diversity_dict[environment].keys())
        distances.remove('diversity_species')
        distances.sort()

        #print(diversity_vs_diversity_dict[environment][distances[0]].keys())

        mean_diveristy = [numpy.mean(diversity_vs_diversity_dict[environment][d]['diversity']) for d in distances]


        upper_ci = [diversity_vs_diversity_dict[environment][d]['mean_diversity_null_upper_ci'] for d in distances]
        lower_ci = [diversity_vs_diversity_dict[environment][d]['mean_diversity_null_lower_ci'] for d in distances]

        upper_ci_constrain_mad = [diversity_vs_diversity_dict[environment][d]['mean_diversity_null_constrain_mad_upper_ci'] for d in distances]
        lower_ci_constrain_mad = [diversity_vs_diversity_dict[environment][d]['mean_diversity_null_constrain_mad_lower_ci'] for d in distances]
        ...
        ax.fill_between(distances, lower_ci, upper_ci, color='dodgerblue', alpha=0.5, label='95% PI', zorder=1)
        ax.fill_between(distances, lower_ci_constrain_mad, upper_ci_constrain_mad, color='orangered', alpha=0.5, label='95% PI, constrain ' + r'$\bar{x}$', zorder=1)
...
fig_name = "%sdiversity_slm_null_%s_emp_phylo.png" % (config.analysis_directory, file_label)
