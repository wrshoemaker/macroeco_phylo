...
for environment_chunk_idx, environment_chunk in enumerate(environment_chunk_all):
    ...
    for environment_idx, environment in enumerate(environment_chunk):
        ...
        diversity_dict = predict_diversity_slm_emp.load_predict_diversity_phylo_dict(environment)

        distances = list(diversity_dict.keys())
        distances.sort()

        mean_diveristy = [diversity_dict[d]['observed'] for d in distances]
        #mean_diveristy_null = [diversity_dict[environment][rank]['mean_diveristy_null'] for rank in diversity_utils.taxa_ranks]
        upper_ci = [diversity_dict[d]['upper_ci'] for d in distances]
        lower_ci = [diversity_dict[d]['lower_ci'] for d in distances]


        ax.plot(distances, mean_diveristy, ls='-', lw=1.5, c='k',  zorder=2)
        ax.scatter(distances, mean_diveristy, c='k', label='Observed',  zorder=3)
        ax.fill_between(distances, lower_ci, upper_ci, color='darkgrey', alpha=0.7, label='95% Prediction interval', zorder=1)
...
fig_name = "%sdiversity_slm_emp_phylo.png" % config.analysis_directory
