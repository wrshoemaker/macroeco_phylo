...
for environment_chunk_idx, environment_chunk in enumerate(environment_chunk_all):
    ...
    for environment_idx, environment in enumerate(environment_chunk):
        ...
        for rank_idx, rank in enumerate(taxa_ranks):
            ...
            for genus_idx, genus in enumerate(all_genera):
                ...
                if len(beta_div_mean)>5:
                    ...
                    mean_beta_div_mean_all.append(numpy.mean(beta_div_mean))
                    cv_beta_div_mean_all_idx.append(True)

                else:
                    cv_beta_div_mean_all.append(0)
                    mean_beta_div_mean_all.append(0)
            ...
            mean_beta_div_mean_all = numpy.asarray(mean_beta_div_mean_all)
            mean_beta_div_mean_all_to_keep = mean_beta_div_mean_all[(cv_beta_div_mean_all_idx) & (error_to_keep_idx)]

            cv_beta_div_mean_vs_error_rho = numpy.corrcoef(numpy.log10(cv_beta_div_mean_all_to_keep), numpy.log10(error_to_keep))[1,0]
            mean_beta_div_mean_vs_error_rho = numpy.corrcoef(numpy.log10(mean_beta_div_mean_all_to_keep), numpy.log10(error_to_keep))[1,0]
            print(rank, 10**numpy.mean(numpy.log10(cv_beta_div_mean_all_to_keep)), cv_beta_div_mean_vs_error_rho)#, mean_beta_div_mean_vs_error_rho)

            print(len(cv_beta_div_mean_all_to_keep))
        ...
        ax.plot(idx_taxa_ranks, richness_vs_error_rho_all, ls='-', lw=1.5, c='k',  zorder=2)
        ax.scatter(idx_taxa_ranks, richness_vs_error_rho_all, c=color_all, label='Observed',  zorder=3)
        ...
        if environment_idx == 0:
            ax.set_ylabel("Corr. b/w mean log fine-grain richness and\nmean log error of coarse-grained AFD", fontsize = 10)
...
fig_name = "%srichnessvs_error_taxon%s.png" % (config.analysis_directory, diversity_utils.get_rarefied_label(rarefied))
