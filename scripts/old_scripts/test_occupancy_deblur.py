...
ax_gamma = plt.subplot2grid((1, 3), (0, 0))
ax_gamma_hyper = plt.subplot2grid((1, 3), (0, 1))
ax_gamma_sample = plt.subplot2grid((1, 3), (0, 2))
...
predicted_occupancy_sample_all = []
for afd_subsample_idx, afd_subsample in enumerate(s_by_s_subsample):
    ...
    predicted_occupancy_sample_all.append(predicted_occupancy)
...
predicted_occupancy_sample_all = numpy.asarray(predicted_occupancy_sample_all)
...
ax_gamma.set_xlabel("Observed occupancy, deblur, subsampled", fontsize = 9)
ax_gamma.set_ylabel("Predicted occupancy, deblur", fontsize = 9)
...
ax_gamma_hyper.set_xlabel("Observed occupancy, deblur, subsampled", fontsize = 9)
ax_gamma_hyper.set_ylabel("Predicted occupancy, deblur, corrected for subsampling", fontsize = 9)




# plot sample, not subsampled


idx_to_keep = (observed_occupany_all>0) & (predicted_occupancy_sample_all > 0)
occupancies = observed_occupany_all[idx_to_keep]
predicted_occupancies = predicted_occupancy_sample_all[idx_to_keep]
...
x, y, z = plot_utils.get_scatter_density_arrays_for_loglog(occupancies, predicted_occupancies)
ax_gamma_sample.scatter(x, y, c=numpy.sqrt(z), cmap='Blues', s=70, alpha=0.9, edgecolors='none', zorder=1)
ax_gamma_sample.set_xscale('log', base=10)
ax_gamma_sample.set_yscale('log', base=10)
ax_gamma_sample.plot([min_,max_],[min_,max_], lw=3,ls=':',c='k',zorder=1)


ax_gamma_sample.set_xlabel("Observed occupancy, deblur, NOT subsampled", fontsize = 9)
ax_gamma_sample.set_ylabel("Predicted occupancy, deblur NOT subsampled", fontsize = 9)
