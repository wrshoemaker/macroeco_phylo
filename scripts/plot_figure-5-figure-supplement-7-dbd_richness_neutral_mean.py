...
for environment_idx, environment in enumerate(diversity_utils.environments_to_keep):
    ...
    for rank_idx, rank in enumerate(diversity_utils.taxa_ranks):
        ...
        slope_all = dbd_dict[environment]['taxon'][rank]['slope_all']
        slope_slm_all = dbd_dict[environment]['taxon'][rank]['slope_neutral_all']

        mean_slope = dbd_dict[environment]['taxon'][rank]['mean_slope']
        mean_slope_slm = dbd_dict[environment]['taxon'][rank]['mean_slope_neutral']
        ...
        taxon_slope_all.extend(dbd_dict[environment]['taxon'][rank]['slope_all'])
        taxon_slope_slm_all.extend(dbd_dict[environment]['taxon'][rank]['slope_neutral_all'])
...
ax_taxon.set_xlabel("Observed mean richness slope", fontsize = 12)
ax_taxon.set_ylabel("Predicted mean richness slope, UNTB", fontsize = 11)
...
for environment_idx, environment in enumerate(diversity_utils.environments_to_keep):
    ...
    for distance_idx, distance in enumerate(distances):
        ...
        slope_all = dbd_dict[environment]['phylo'][distance]['slope_all']
        slope_slm_all = dbd_dict[environment]['phylo'][distance]['slope_neutral_all']

        mean_slope = dbd_dict[environment]['phylo'][distance]['mean_slope']
        mean_slope_slm = dbd_dict[environment]['phylo'][distance]['mean_slope_neutral']
        ...
        phylo_slope_all.extend(dbd_dict[environment]['phylo'][distance]['slope_all'])
        phylo_slope_slm_all.extend(dbd_dict[environment]['phylo'][distance]['slope_neutral_all'])
...
ax_phylo.set_xlabel("Observed mean richness slope", fontsize = 12)
ax_phylo.set_ylabel("Predicted mean richness slope, UNTB", fontsize = 11)
...
fig_name = "%sfigure-5-figure-supplement-7-dbd_richness_neutral_mean.png" % (config.analysis_directory)
