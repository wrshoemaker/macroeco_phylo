...
for environment_idx, environment in enumerate(environments_to_keep):
    ...
    for distance_idx, distance in enumerate(distances):
        ...
        x, y, z = plot_utils.get_scatter_density_arrays_for_loglog(sum_var_all, coarse_grained_var_all)
        ...
        min_ = min(sum_var_all+coarse_grained_var_all)
        max_ = max(sum_var_all+coarse_grained_var_all)
...
fig_name = "%sfigure-2-figure-supplement-11-fine_vs_coarse_variance_phylo%s.png" % (config.analysis_directory, diversity_utils.get_rarefied_label(rarefied))
